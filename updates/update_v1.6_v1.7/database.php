...
if (isset($_POST["btn_admin"])) {
    ...
    if (mysqli_connect_errno()) {
        ...
        mysqli_query($connection, "UPDATE settings SET version = 'v1.7' WHERE id = 1;");
        mysqli_query($connection, "ALTER TABLE `settings` ADD `terms_condition` LONGTEXT NULL DEFAULT NULL AFTER `layout`;");
        mysqli_query($connection, "ALTER TABLE `settings` ADD `enable_blog` INT NULL DEFAULT '1' AFTER `enable_paypal`;");

        mysqli_query($connection, "ALTER TABLE `payment` ADD `expire_on` DATE NULL DEFAULT NULL AFTER `created_at`;");
        mysqli_query($connection, "ALTER TABLE `users` ADD `email_verified` INT NULL DEFAULT '0' AFTER `enable_followers`, ADD `verify_code` VARCHAR(255) NULL DEFAULT NULL AFTER `email_verified`;");
        mysqli_query($connection, "ALTER TABLE `settings` ADD `mail_protocol` VARCHAR(255) NULL DEFAULT NULL AFTER `paypal_email`, ADD `mail_title` VARCHAR(255) NULL DEFAULT NULL AFTER `mail_protocol`, ADD `mail_host` VARCHAR(255) NULL DEFAULT NULL AFTER `mail_title`, ADD `mail_port` VARCHAR(255) NULL DEFAULT NULL AFTER `mail_host`, ADD `mail_username` VARCHAR(255) NULL DEFAULT NULL AFTER `mail_port`, ADD `mail_password` VARCHAR(255) NULL DEFAULT NULL AFTER `mail_username`, ADD `enable_email_verify` INT(255) NULL DEFAULT '0' AFTER `mail_password`;");
      ...
      $redir = str_replace('updates/update_v1.6_v1.7/', '', $redir);
